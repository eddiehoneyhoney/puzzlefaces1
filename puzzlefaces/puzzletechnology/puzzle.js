...
(function(object){
  ...
  document.observe('dom:loaded', function(){
    ...
    function flip(element, image){
      ...
      img.morph('width:0px;left:48px;', {
        duration: .2, transition: 'easeInCubic',
        after: function(){
          if(image) img.src = image;
          img.morph('width:50px;left:0px', {
            duration: .2, transition: 'easeOutExpo'
    ...
    function updatescore(){
      $('score').update(score).setStyle('color: blue').morph('color:#aaa', 2);
    }
...
window.speechSynthesis.speak(msg);
      ...
      window.location = "http://puzzlefaces.xama.online/puzzlewonders/";
      //shuffle.delay(17);
    }
    
      
      function loose(){
            var msg = new SpeechSynthesisUtterance('You Loose Try again Please Thank you');
msg.lang = 'en-US';
window.speechSynthesis.speak(msg);
      cards.each(function(card){
        card.morph('opacity:1');
